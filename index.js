...
client.on(Events.MessageCreate, async (message) => {
    ...
    if (command === "serverinfo") {
        ...
        const alreadySent = sentMessages.some(msg => {
            return msg.embeds.length > 0 && 
                   msg.embeds[0].title === serverEmbed.title && 
                   JSON.stringify(msg.embeds[0].fields) === JSON.stringify(serverEmbed.fields); // Compara los campos del embed
        });
...
client.on(Events.GuildMemberAdd, (member) => {
    const logChannel = member.guild.channels.cache.get('1305077832329986088');
    sendLog(logChannel, `Nuevo miembro: ${member.user.tag}`);
});
